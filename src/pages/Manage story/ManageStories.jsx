...
const ManageStories = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axiosSecure.delete(`/stories/${id}?email=${user?.email}`);
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {stories.map((story) => (
          ...
          >
            ...
            <h2 className="text-xl font-semibold text-lime-300">{story.title}</h2>
            <p className="text-sm mt-2 break-all">{story.story}</p>

            {/* Show edit/delete only for own story */}
